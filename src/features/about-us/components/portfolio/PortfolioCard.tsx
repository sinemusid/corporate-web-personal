...
export const PortfolioCard: React.FC<PortfolioCardProps> = ({ item, onClick }) => {
  return (
    <article
      onClick={onClick}
      className="group relative flex flex-col rounded-3xl bg-white border border-slate-200/80 hover:border-blue-500 transition-all duration-300 overflow-hidden shadow-xs hover:shadow-xl hover:-translate-y-1 cursor-pointer"
    >
      ...
      <div className="relative w-full aspect-[16/10] overflow-hidden bg-slate-100">
        ...
        <div className="absolute top-3.5 left-3.5 right-3.5 flex items-center justify-between z-10">
          <span className="px-3 py-1 rounded-full text-xs font-mono font-bold tracking-wider text-white bg-slate-950/60 border border-white/20 backdrop-blur-md shadow-xs">
            {item.category}
          </span>
          <span className="px-2.5 py-0.5 rounded-md text-xs font-mono font-medium text-slate-200 bg-slate-950/70 border border-white/10 backdrop-blur-md">
            {item.year}
          </span>
        </div>
      </div>
